...
2.  Take two samples each of size 50 from the `area` data and call them `samp1` and `samp2` respectively. How does the mean of `samp1` compare with the mean of `samp2`? Suppose we took two more samples, one of size 100 and one of size 1000. Which would you think would provide a more accurate estimate of the population mean? **[3 marks]**
...
4.  Recreate sampling distribution histograms for the mean area for samples of three different sizes (that each differ by at least 30). As the sample size increases, what happens to the center of mass of the distribution?  Describe what happens to the spread (dispersion) as the sample size increases? **[6 marks]**
...
5.  Take 1000 samples each of size 15 from `price/1e3`, calculate the variance of each sample. Present the distribution of the sample variances in a histogram. Take 1000 samples of size 30 from `price/1e3`, calculate the variance of each sample. Present the distribution of the sample variances in a histogram. Comment on the underlying distribution for the sample variance. What happens to the mean of the sample variance as the sample size increases? *Your solution to this question may be circulated among the class for peer feedback; please make sure it appears on a separate page.* **[6 marks]**
...
This work is derivative of OpenIntro Labs and was modified by Eric Hall. The original labs were adapted for OpenIntro by Andrew Bray and Mine Çetinkaya-Rundel from labs written by Mark Hansen of UCLA Statistics.
