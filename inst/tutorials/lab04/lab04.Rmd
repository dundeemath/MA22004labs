---
title: "MA22004/MA52008 Lab 4: Confidence intervals"
...
:::{.noteblock}
The report for Lab 4 consists of answers to exercises 1-5 in the last section of this tutorial. Upload the submission to [Gradescope](https://www.gradescope.com) as a PDF; an `rmarkdown` template called *MA22004/MA52008 Lab Report* is provided. Further instructions on using the class template to produce a nice lab report containing data analysis and text are in Section "RStudio and lab reports" of Lab 1. 
:::
...
Bootstrapping is a simulation-based method for estimating uncertainty in a statistic (like a mean or proportion) using only the observed sample. The idea is to treat the observed sample as our best approximation to the population, and then repeatedly resample the data with replacement to mimic the process of taking many samples from the population.

In essence, bootstrapping approximates the population distribution using the empirical distribution of the observed sample, then resamples from that approximation with replacment. That is, in the nonparametric bootstrap, we treat the observed sample as our best approximation to the population. We then create many ‘new samples’ by resampling the observed data with replacement (usually keeping the same sample size). To bootstrap, we:
...
-   *Step 2.* Calculate the statistic of interest on that resample - a statistic such as mean, median, proportion, slope, etc. computed on the bootstrap sample.
-   *Step 3.* Repeat steps (1) and (2) many times to build a bootstrap distribution - a distribution of bootstrap statistics.
-   *Step 4.* Form a confidence interval, commonly using the **percentile method**, e.g., for 95% take the 2.5% and 97.5% percentiles of the bootstrap distribution.

The bootstrap method does not require normality, but it does require your sample to be representative of the population you are trying to learn about. Bootstrapping works best when the observations are an independent random sample from the population (or at least reasonably representative).

:::{.warningblock}
Bootstrapping assumes the sample is representative of the population and that observations are (approximately) independent.
:::

:::{.importantblock}
If the original sample is biased or unrepresentative, bootstrapping will reproduce that bias.
:::
...
+-------------+--------------------------------------------------------------------------------------------------------------------------------------------+
| `calculate` | Compute the sample statistic (e.g., mean) on each resample to form the bootstrap distribution |
+-------------+--------------------------------------------------------------------------------------------------------------------------------------------+
...
:::{.exerciseblock}
Included with the lab is a non-standard function called `plot_ci`; use the command `plot_ci(lower_vector, upper_vector, mean(population$area))` to plot fifty (50) 95% confidence intervals for the true mean area from the **Ames House Data**. What proportion of your confidence intervals include the population mean? Is this proportion exactly equal to the confidence level? If not, explain why.
...
1.  Using `ggplot`, plot and describe the distribution of `samp$area`, where `samp <- sample_n(ames, 60)`. What would you say is the "typical" size within your sample? Also, state precisely what you interpreted "typical" to mean (e.g., reference a summary statistic). **[3 marks]**

2.  Would you expect another student's distribution for `samp$area` (with `samp <- sample_n(ames, 60)`) to be identical to yours? Would you expect it to be similar? Why or why not? **[3 mark]**
...
4.  Included with the lab is a non-standard function called `plot_ci`. Using the approach demonstrated earlier in the lab, construct fifty 90% confidence intervals for the true mean area of the **Ames House Data**, and use `plot_ci` to plot them. What proportion of your confidence intervals includes the population mean? Is this proportion exactly equal to 90%? If not, explain why. **[5 marks]**
...
5.  Using code from the **infer** package and one simple random sample of size 60 from `ames` (e.g. `samp <- sample_n(ames, 60)`), find a 90% bootstrap confidence interval for the proportion of Ames houses that are single-family homes. Clearly state which variable you used and what value you treated as "single-family" (the `success = ...` level), and interpret your ocnfidence interval in context. **[6 marks]**

*Hint: Use `names(ames)` to find the relevant categorical variable, and `count(ames, <variable>)` to see the levels so you can choose the correct `success = "..."`.*
