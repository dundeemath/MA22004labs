...
:::{.exerciseblock}
Make a boxplot (using `geom_boxplot()`) of `weight` by the new categorical variable `physical_3plus`. Is there a relationship between engaging in physical activity for at least 3 days a week and weight? What did you expect and why?
...
:::{.exerciseblock}
Consult the sections of the lecture notes on hypothesis testing (or your favorite statistics text). Are all conditions necessary for making an inference satisfied? Comment on each. You can compute the group sizes with the `summarize` command as above by defining a new variable with the definition `n()`.
...
1.  Create a new categorical variable `physical_3plus` denoting those who engaging in physical activity for at least 3 days a week (e.g., appropriate levels would be "no", "yes", and "NA"). Using `ggplot`, make a boxplot of `height` by `physical_3plus`. Is there a relationship between engaging in physical activity for at least 3 days a week and height? What did you expect and why? **[3 marks]**

2.  Calculate a 90% confidence interval for the average respondent height in meters (`height`) and interpret it in context. Use both the `infer` package and normality theory and compare the two interval estimates.  **[5 mark]**
...
3.  Conduct a hypothesis test evaluating whether the average height is different for those who exercise at least three times a week and those who don't at confidence level $0.01$. Please use the `infer` package to conduct the test.  **[5 marks]**
