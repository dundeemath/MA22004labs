...
```{r inf-weight-habit-ht-initial, tidy=FALSE, warning = FALSE, echo = TRUE, eval = TRUE}
obs_diff <- yrbss_exercise %>%
  specify(weight ~ physical_3plus) %>%
  calculate(stat = "diff in means", order = c("yes", "no"))
obs_diff$stat
...
```{r inf-weight-habit-ht-null, tidy=FALSE, warning = FALSE, echo = TRUE, eval = TRUE}
null_dist <- yrbss_exercise %>%
  specify(weight ~ physical_3plus) %>%
  hypothesize(null = "independence") %>%
  generate(reps = 1000, type = "permute") %>%
...
2.  Calculate a 90% confidence interval for the average height in meters (`height`) and interpret it in context. **[3 mark]**
...
3.  Conduct a hypothesis test evaluating whether the average height is different for those who exercise at least three times a week and those who don't. **[5 marks]**
